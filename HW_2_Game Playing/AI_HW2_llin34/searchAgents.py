...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # Visited corner = [LeftBot, LeftTop, RightBot, RightTop]
        self.visitedCorner = [False, False, False, False]
        # state = ((position, visitedCorner), action, cost)
        self.startingState = (self.startingPosition, self.visitedCorner)
    ...
    def isGoalState(self, state):
        ...
        return visitedCorner == [True, True, True, True]
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            # Get the coordinates of successors.
            position, visitedCorner = state
            (x, y) = position
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                # The coordinates is not a wall, then add this node as a successor.
                newVisitedCorner = visitedCorner[:]
                for i in xrange(len(self.corners)):
                    if (nextx, nexty) == self.corners[i]:
                        newVisitedCorner[i] = True
                successors.append((((nextx, nexty), newVisitedCorner), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    # Calculate cost. 
    # Find the longest vertical and horizontal distances among the unvisited 
    # corners and the current position.
    heuValue = 0
...
def foodHeuristic(state, problem):
    ...
    # This food heuristic uses the farest maze distance of food as the cost of current position.
    farestDist = 0
    if len(foodList) > 0:
        ...
        # Set the first farest maze distance. 
        if (position, foodList[0]) in problem.heuristicInfo:
            farestDist = problem.heuristicInfo[(position, foodList[0])]
        else:
            farestDist = mazeDistance(position, foodList[0], problem.startingGameState)
            problem.heuristicInfo[(position, foodList[0])] = farestDist

        # Find the farest maze distance to the farest food.
